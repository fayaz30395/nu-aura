...
**Locked constraints** (from root `AGENTS.md`, non-negotiable):
...
cat AGENTS.md .Codex/AGENTS.md MEMORY.md 2>/dev/null
...
CONSTRAINTS: Do NOT write to source. Stack is locked (see /AGENTS.md). SuperAdmin bypass must remain intact.
...
CONSTRAINTS:
  - Stack locked per /AGENTS.md (no alternatives).
...
- **Run from Codex at repo root.** Cowork lacks direct filesystem access.
- **Controller state lives in files** — re-read `AGENTS.md`, `MEMORY.md`, `TASKS.md`, `GAP_REPORT.md` at every session start.
