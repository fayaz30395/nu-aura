...
function EmployeeCard({
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={`p-2 bg-white rounded border border-gray-200 shadow-sm cursor-pointer hover:shadow-md transition-all text-xs ${
        isDragging ? 'opacity-50' : ''
      } ${!readOnly ? 'hover:border-blue-400' : ''}`}
    >
      <div className="flex items-start justify-between gap-1">
        <div className="flex-1 min-w-0">
          <p className="font-medium text-gray-900 truncate">{employee.employeeName}</p>
          <p className="text-gray-500 truncate text-xs">{employee.designation}</p>
        </div>
        {!readOnly && <GripVertical className="h-3 w-3 text-gray-400 flex-shrink-0" />}
      </div>
...
function MatrixCell({
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className={`border border-gray-300 p-2 rounded-lg min-h-[180px] transition-all ${
        isDragOver ? 'bg-blue-50 border-blue-400 border-2' : 'bg-gray-50'
      } ${cellInfo.color} bg-opacity-10`}
    >
      <div className="mb-2 pb-2 border-b border-gray-200">
        <h4 className="text-xs font-semibold text-gray-900 leading-tight">{cellInfo.title}</h4>
        <p className="text-xs text-gray-600 mt-0.5 leading-tight">{cellInfo.description}</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white p-4 rounded-lg border border-gray-200">
        <h3 className="text-sm font-semibold text-gray-900 mb-3">9-Box Grid Legend</h3>
      ...
      {/* Summary Statistics */}
      <div className="bg-white p-4 rounded-lg border border-gray-200">
        <h3 className="text-sm font-semibold text-gray-900 mb-3">Distribution Summary</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <div className="text-xs text-gray-500">Solid Contributors</div>
          </div>
          <div>
