...
Pixel reference for every screen and key granular state. Diff your implementation against these.

| # | File | Shows |
|---|---|---|
| 01 | `01-login.png` | Login — brand split + form |
| 02 | `02-dashboard-light.png` | Dashboard (light) |
| 03 | `03-dashboard-dark.png` | Dashboard (dark) — theme parity |
| 04 | `04-employees.png` | Employees directory |
| 05 | `05-employees-profile-slideover.png` | **Granular:** profile slide-over (Overview tab) |
| 06 | `06-employees-bulk-select.png` | **Granular:** bulk-select action bar |
| 07 | `07-approvals.png` | Approvals split master-detail |
| 08 | `08-attendance.png` | Attendance weekly heatmap |
| 09 | `09-leave.png` | Leave — balances + month calendar |
| 10 | `10-payroll.png` | Payroll — run banner + pipeline |
| 11 | `11-assets.png` | Assets inventory |
| 12 | `12-benefits.png` | Benefits plan grid |
| 13 | `13-reports.png` | Reports gallery + schedule |
| 14 | `14-settings-roles.png` | Settings — **RBAC console** (role + permission matrix) |
| 15 | `15-command-palette.png` | **Granular:** ⌘K command palette |

> Capture note: these are reference stills from the prototype; one or two may show a minor cosmetic difference from the latest build (an icon mid-swap, or text that has since been set to non-wrapping). The live prototype and the source files are the source of truth. Production should import `lucide-react` directly, which renders icons synchronously.
...
### 1. Login / Landing — `LoginPage.jsx`
Two-column grid `1.05fr / 1fr`; below 920px the brand panel hides.
- **Left brand panel (the "catchy" landing):** dark gradient `150deg #0c1120 → #14193a → #2a1d49` with two blurred color orbs (red-orange + purple), radial color washes, and a faint 44px grid masked radially. Content stack: logo (46px gradient mark + "NU-AURA / by NULogic Technologies") → **eyebrow pill** (green live-dot + "The enterprise people platform") → headline **"One login. Your whole people stack."** (42px/700) where *"people stack."* uses an orange→purple **text gradient** → lede → **metric strip** (1,248 employees · 4-in-1 products · 99.98% uptime, divider-separated) → 2×2 product cards (hover-lift) → **trust row** (overlapping avatar stack + "1.2k+" + "Trusted by 1,200+ people teams worldwide") → **compliance badges** (SOC 2 Type II · GDPR · ISO 27001, each with a green shield-check).
...
- **Roles & permissions** (default) — a full **RBAC console** (`RbacPanel`), not just a table. Two-pane: *left* = selectable roles list (color tone-dot + name + "N users · M perms", active row tinted + chevron); *right* = selected-role detail: header (tone dot + name + scope badge + Clone/Edit), description, three summary tiles (Users / Permissions / Modules N/9), then a **module permission matrix** — each of 9 modules (Employees, Attendance, Leave, Payroll, Benefits, Assets, Reports, Recruitment, Settings) shows its access level via a 4-segment **None → View → Edit → Manage** bar (active segment filled, escalating monochrome blue; "Manage" = solid accent) plus the level label. Selecting a role re-renders the matrix from `NU.rolePerms`. Legend top-right; header "9 RBAC roles · 1,283 users · 512 granular permissions"; Audit log + New role actions.
