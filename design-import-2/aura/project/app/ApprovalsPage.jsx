...
function ApprovalsPage() {
  ...
  function act(id, v) {
    setDone(d => ({ ...d, [id]: v }));
    const r = NU.approvals.find(a => a.id === id);
    if (v === 'approved') window.nuToast('Request approved', { msg: `${r.type} · ${r.who} has been notified.`, type: 'ok' });
    else window.nuToast('Request declined', { msg: `${r.type} · ${r.who} has been notified.`, type: 'err' });
  }
    ...
    <div className="page">
      <div className="phead">
        ...
        <div className="pactions">
          <Button variant="ghost" icon="check-check" onClick={() => { const leave = NU.approvals.filter(a=>a.type==='Leave'); setDone(d => { const n={...d}; leave.forEach(a=>n[a.id]='approved'); return n; }); window.nuToast('All leave approved', { msg: `${leave.length} leave requests approved.`, type: 'ok' }); }}>Approve all leave</Button>
