            ...
            {NU.approvals.slice(0,4).map((a, i) => (
              <div key={a.id} style={{ display:'flex', alignItems:'center', gap:13, padding:'13px 20px', borderTop: i? '1px solid var(--border-soft)':'0' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <Button variant="ghost" size="sm" onClick={() => window.nuToast('Request declined', { msg: `${a.who} · ${a.type}`, type:'err' })}>Decline</Button>
                  <Button variant="primary" size="sm" icon="check" onClick={() => window.nuToast('Request approved', { msg: `${a.who} · ${a.type}`, type:'ok' })}>Approve</Button>
                </div>
              </div>
            ))}
          </div>
