...
SELECT id, email, first_name, last_name, tenant_id FROM users ORDER BY created_at;
...
WHERE u.email = 'fayaz.m@nulogic.io'
  AND NOT EXISTS (
    SELECT 1 FROM roles r WHERE r.code = 'SUPER_ADMIN' AND r.tenant_id = u.tenant_id
  )
LIMIT 1;
...
WHERE u.email = 'fayaz.m@nulogic.io'
  AND a.code = 'HRMS'
  AND NOT EXISTS (
    SELECT 1 FROM user_app_access uaa WHERE uaa.user_id = u.id AND uaa.application_id = a.id
  );
...
WHERE u.email = 'fayaz.m@nulogic.io'
  AND NOT EXISTS (
    SELECT 1 FROM user_app_roles uar WHERE uar.user_app_access_id = uaa.id AND uar.role_id = ar.id
  );
