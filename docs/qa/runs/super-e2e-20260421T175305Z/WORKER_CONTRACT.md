...
4. Auth rate limit: 5 logins/min. Stagger — sleep `${W_INDEX}*12` seconds before your FIRST navigate.
5. On every page:
...
6. On each FAIL, append a JSON line to `$RUN_DIR/bugs.jsonl` via:
   ...
   ```
   Schema: `{"bug_id":"BUG-W{N}-{n}","worker":"W{N}","tab_id":...,"severity":"P0|P1|P2|P3","category":"rbac|api|ui|regression|perf","title":"...","url":"...","endpoint":"...","http_status":...,"repro":["..."],"evidence":"workers/w{N}/screenshots/...","detected_at":"<iso>"}`
...
- SuperAdmin: `fayaz@nulogic.io` / `Welcome@123`  ⚠ may be locked out for 15min after failed Password@123 attempts
