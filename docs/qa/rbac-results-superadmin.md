...
1. **`/api/v1/employees/1`** returned 400 — the endpoint expects a UUID path variable, not an
   integer. When tested with a valid UUID, it returned 200. This is correct API behavior, not an
   RBAC issue.
...
Super Admin RBAC is working correctly. The `SUPER_ADMIN` role bypasses all permission checks as
designed, granting unrestricted access across all 49 tested endpoints spanning NU-HRMS, NU-Hire,
NU-Grow, NU-Fluence, and platform admin modules.
