...
- **Expected non-200: 15**
  - 400 Missing Parameter: caller-side responsibility, not a bug (e.g. `/calendar/events/range`
    requires `startTime`)
  - 405 Method Not Allowed: POST-only endpoints I tested as GET (e.g. `/feedback`,
...
1. Profile `/dashboards/manager` — check `ManagerDashboardService.java` (572 lines) for N+1
   patterns, add `@Cacheable`
...
4. Re-run this sweep as other roles (Employee, Team Lead, HR Admin) to validate RBAC — couldn't
   complete without Chrome
