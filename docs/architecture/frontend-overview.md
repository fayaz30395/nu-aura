...
- **Timeout**: 30 seconds
- **401 Handling**: Shared refresh Promise prevents race conditions; auto-retry after refresh;
  fallback redirect to `/auth/login?reason=expired`
...
Located in `frontend/lib/api/` and `frontend/lib/services/`. Every backend module has a
...
- **Employee**: useEmployees, useEmployee, useMyEmployee, useManagers, useEmployeeHierarchy,
  useSubordinates
...
- **Performance**: usePerformanceReviews, useReviewCreate/Submit, useFeedback360,
...
- **Plus**: expenses, benefits, loans, travel, training, compensation, tax, analytics, contracts,
  recognition, surveys, calendar, integrations, learning, wellness, admin
