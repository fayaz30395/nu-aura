...
1. **DEF-43/44 guards are placed after hooks but before render.** This is correct — React rules of
   hooks require all hooks to be called unconditionally, so the guard must come after `useForm`,
   `useQuery`, etc. but before any JSX return. Both files follow this pattern correctly.

2. **Data fetching still executes for unauthorized users** in both DEF-43 and DEF-44 (React Query
   hooks fire before the guard). The backend will 403 these requests, so no data leaks. The frontend
   guard prevents the UI from ever rendering the response even if the query somehow succeeds. This
   is defense-in-depth.

3. **Password reset page has no `'use client'` boundary issue.** The `useSearchParams` call is
   inside `ResetPasswordForm` (a client component), wrapped in `Suspense` by the page-level
   `ResetPasswordPage` default export. This is the correct Next.js App Router pattern.

4. **DEF-45 (Won't Fix) is acceptable.** The import page's file drag-and-drop uses `useState` for
   file state, which is the standard React pattern for file inputs. RHF `Controller` would add
