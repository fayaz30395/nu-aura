...
- Pages 9 and 15 already had `useAuth()` authentication checks; permission gates were layered on top.
...
- MODULE:MANAGE hierarchy in `hasPermission()` means users with MANAGE automatically pass VIEW checks.
