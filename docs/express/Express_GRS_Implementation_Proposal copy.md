...
flowchart TB
    ...
    subgraph Edge["Edge & API"]
        ...
        Exp["Experiment + Feature Flags"]
    end

    subgraph Runtime["Express GRS Runtime"]
        Parse["Query Understanding<br/>spell fix, synonyms, intent, entities"]
        Retrieve["Candidate Retrieval<br/>keyword + semantic + rules"]
        Features["Context Builder<br/>profile, size, price, geo, inventory"]
        Rank["Ranking Pipeline<br/>business rules + ML scoring"]
        Assemble["Response Assembler<br/>results, facets, badges, reasons"]
    end

    subgraph OnlineData["Online Serving Data"]
        Catalog["Catalog + Search Index"]
        Vector["Vector Index / Embeddings"]
        Profile["User Profile Store"]
        Inventory["Inventory + Availability"]
        Geo["Geo + Weather Context"]
        Cache["Redis Feature/Result Cache"]
    end

    subgraph Learning["Feedback & Model Loop"]
        Events["Clickstream / Search Events"]
    ...
    API --> Auth
    API --> Exp
    API --> Parse --> Retrieve --> Features --> Rank --> Assemble --> API
    ...
    class Web,App,Merch clientClass
    class CDN,API,Auth,Exp gatewayClass
    class Parse,Retrieve,Features,Rank,Assemble orchClass
    class Serve,Train mlClass
    class Catalog,Vector,Profile,Inventory,Geo,Cache,Events,Stream,Warehouse,FeatureStore dataClass
```
...
flowchart TB
    subgraph Inputs["Location Inputs"]
        Request["Search / Category Request"]
        IP["IP Geolocation"]
        Address["Saved Address / Last Delivery PIN"]
        Device["Device Locale / App GPS if allowed"]
    end

    subgraph Context["Geo Context Resolution"]
        Resolver["Location Resolver<br/>pick best signal + confidence"]
        Region["Region Mapper<br/>city -> climate zone -> market cluster"]
        Weather["Weather Snapshot<br/>current + 3-5 day forecast"]
        Calendar["Season + Holiday Calendar"]
    end

    subgraph Decision["Merchandising Decision Engine"]
        Rules["Rule Evaluation<br/>seasonal, regional, weather, campaign"]
        Examples["Representative Outputs<br/>Chicago + snow = boots up<br/>Miami + heat = resortwear up"]
        Safeguards["Guardrails<br/>inventory, margin, exclusions"]
    end

    subgraph Actions["Search / PLP Actions"]
        Boost["Boost ranked products/categories"]
        Suppress["Suppress irrelevant assortments"]
        Facets["Pre-apply facets and banners"]
        Explain["Attach geo reason codes for analytics"]
    end
    ...
    class Request,IP,Address,Device inputClass
    class Resolver,Region,Weather,Calendar processClass
    class Rules,Examples,Safeguards ruleClass
    class Boost,Suppress,Facets,Explain outputClass
```
...
flowchart TB
    subgraph Input["User Query"]
        RawQuery["User types: blk maxi drss"]
    end

    subgraph Normalize["Normalization Layer"]
        Clean["Lowercase, trim, tokenize"]
        Spell["Spell correction<br/>blk -> black, drss -> dress"]
        Synonyms["Synonyms / aliases<br/>trainers = sneakers"]
    end

    subgraph Understand["Understanding Layer"]
        Intent["Intent classifier<br/>product search vs attribute search"]
        Entity["Entity extraction<br/>color:black, style:maxi"]
        Expand["Query expansion<br/>dress, gown, maxi dress"]
    end

    subgraph Build["Search Plan"]
        Structured["Structured search object"]
        Semantic["Semantic embedding vector"]
        Rules["Fallback / no-result rewrites"]
    end

    subgraph Execute["Search Execution"]
        Lexical["Lexical retrieval"]
        Vector["Semantic retrieval"]
        Merge["Blend + dedupe + send to ranker"]
    end

    subgraph Output["Representative Final Query"]
        FinalQuery["text:black maxi dress<br/>category:dresses<br/>filters:{color:black, length:maxi}"]
    end

    RawQuery --> Clean --> Spell --> Synonyms
    Synonyms --> Intent
    Synonyms --> Entity
    Synonyms --> Expand
    Intent --> Structured
    ...
    class Clean,Spell,Synonyms processClass
    class Intent,Entity,Expand,Structured,Semantic,Rules nlpClass
    class Lexical,Vector,Merge,FinalQuery outputClass
```
