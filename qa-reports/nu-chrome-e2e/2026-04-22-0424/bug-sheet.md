...
Backend RBAC **enforced correctly** on every tested admin endpoint (permissions/roles always 403 for non-admin).

**Observation (not a bug):** MANAGER and TEAM_LEAD receive full tenant employee list (31 rows, includes employees with different `managerId`). Root cause: both roles carry `EMPLOYEE:VIEW_ALL` permission seeded at role creation. If the policy intent is "team only", drop `EMPLOYEE:VIEW_ALL` from MANAGER/TEAM_LEAD role_permissions and rely on `EMPLOYEE:VIEW_TEAM` scope. Flagged for policy owner review.
