...
import { RefreshCw, Home, HardDrive } from 'lucide-react';
...
export default function NuDriveError({ error, reset }: ErrorProps) {
  useEffect(() => {
    handleError(error, { source: 'nu-drive-error-boundary', digest: error.digest });
  }, [error]);
        ...
        <Card className="w-full max-w-md bg-[var(--bg-card)]">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => (window.location.href = '/nu-drive')}
                className="w-full"
              >
