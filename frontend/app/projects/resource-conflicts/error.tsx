...
export default function ResourceConflictsError({ error, reset }: ErrorProps) {
  useEffect(() => {
    handleError(error, { source: 'resource-conflicts-error-boundary', digest: error.digest });
  }, [error]);
        ...
        <Card className="w-full max-w-md bg-[var(--bg-card)]">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => (window.location.href = '/projects/resource-conflicts')}
                className="w-full"
              >
                <AlertTriangle className="mr-2 h-4 w-4" />
                Back to Resource Conflicts
              </Button>
              <Button
                variant="outline"
