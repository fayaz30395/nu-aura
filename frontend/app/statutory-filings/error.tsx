...
export default function StatutoryFilingsError({error, reset}: ErrorProps) {
  useEffect(() => {
    handleError(error, {source: 'statutory-filings-error-boundary', digest: error.digest});
  }, [error]);
        ...
        <Card className="w-full max-w-md bg-[var(--bg-card)]">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => (window.location.href = '/statutory-filings')}
                className="w-full"
              >
                <Grid className="mr-2 h-4 w-4"/>
                Back to Statutory Filings
              </Button>
              <Button
                variant="outline"
