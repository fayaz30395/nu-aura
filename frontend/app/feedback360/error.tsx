...
export default function Feedback360Error({ error, reset }: ErrorProps) {
  useEffect(() => {
    handleError(error, { source: 'feedback360-error-boundary', digest: error.digest });
  }, [error]);
        ...
        <Card className="w-full max-w-md bg-[var(--bg-card)]">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => (window.location.href = '/feedback360')}
                className="w-full"
              >
