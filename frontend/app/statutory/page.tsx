...
import {
  useActiveESIConfigs,
  useActivePFConfigs,
  useMonthlyStatutoryContributions,
  usePTSlabsByState
} from '@/lib/hooks/queries/useStatutory';
                    ...
                    <Table.Tbody>
                      {pfConfigs.length > 0 ? pfConfigs.map(config => (
                        <Table.Tr key={config.id}>
                          ...
                          <Table.Td>
                            <Badge
                              color={config.isActive ? 'green' : 'gray'}>{config.isActive ? 'Active' : 'Inactive'}</Badge>
                          </Table.Td>
                        </Table.Tr>
                      )) : (
                        <Table.Tr><Table.Td colSpan={5} align="center">No active PF configurations
                          found</Table.Td></Table.Tr>
                      )}
                    </Table.Tbody>
                  </Table>
                </Card>
              ...
              <Table>
                ...
                <Table.Tbody>
                  {esiConfigs.length > 0 ? esiConfigs.map(config => (
                    ...
                    <Table.Tr><Table.Td colSpan={4} align="center">No active ESI configurations
                      found</Table.Td></Table.Tr>
                  )}
                </Table.Tbody>
              </Table>
                    ...
                    <Table striped highlightOnHover withTableBorder withColumnBorders fz="xs">
                      ...
                      <Table.Tfoot>
                        <Table.Tr style={{backgroundColor: 'var(--bg-surface)'}}>
                          ...
                          <Table.Td ta="right"
                                    fw={700}>{fmt(totals.pfEmp + totals.esiEmp + totals.pt + totals.tds + totals.vpf)}</Table.Td>
                        </Table.Tr>
                      </Table.Tfoot>
                    </Table>
                  </div>
