    ...
    <AppLayout activeMenuItem="performance">
      <PermissionGate permission={Permissions.REVIEW_VIEW} fallback={
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[var(--text-muted)] text-sm mt-1">You do not have permission to view Performance
            Revolution.</p>
        </div>
      }>
        <div className="p-6 space-y-8 max-w-[1600px] mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1
                className='text-xl font-bold skeuo-emboss bg-clip-text text-transparent bg-gradient-to-r from-accent-700 to-accent-600'>
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => {
                okrGraphQuery.refetch();
                performanceSpiderQuery.refetch();
              }} leftIcon={<RefreshCw className="h-4 w-4"/>}>
                Refresh
              </Button>
              <Button variant="primary" size="sm" className="btn-primary opacity-50 cursor-not-allowed" disabled
                      title="Coming soon" leftIcon={<Share2 className="h-4 w-4"/>}>
                Share Progress
              </Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <Card
              className='lg:col-span-8 overflow-hidden bg-[var(--bg-secondary)] border-none text-inverse relative h-[600px]'>
              <div
                className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(59,130,246,0.1),transparent)]"/>
              <CardHeader className="relative z-10 border-b border-white/5">
                <div className="row-between">
                  <div>
                    <CardTitle className='text-inverse flex items-center gap-2'>
                      <Target className='h-5 w-5 text-accent'/> OKR Alignment Galaxy
                    </CardTitle>
                    <CardDescription className="text-[var(--text-muted)]">Visual mapping of objectives from Company to
                      Individual</CardDescription>
                  </div>
              ...
              <CardContent className="h-full relative flex items-center justify-center">
                ...
                <div className="relative w-full h-full">
                     ...
                     `}>
                        <div className="text-center">
                          ...
                          <div className="w-8 h-1 bg-white/10 rounded-full mt-1 overflow-hidden mx-auto">
                            <div className='h-full bg-accent-subtle' style={{width: `${node.progress}%`}}/>
                          </div>
                        </div>
                        {/* Tooltip on hover */}
                        <div
                          className="absolute invisible group-hover:visible bottom-full mb-2 left-1/2 -translate-x-1/2 bg-[var(--bg-elevated)] text-[var(--text-primary)] p-2 rounded text-xs whitespace-nowrap z-50 shadow-[var(--shadow-dropdown)]">
                  ...
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-0 opacity-20">
                    <Hexagon className='h-96 w-96 text-accent animate-pulse'/>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="lg:col-span-4 bg-[var(--bg-card)]">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Users className='h-5 w-5 text-accent'/> 360° Competency Radar
                </CardTitle>
              ...
              <CardContent>
                ...
                <div className="flex justify-center gap-4 mt-6">
                  <div className="flex items-center gap-1.5">
                    <div className='w-3 h-1.5 rounded-full bg-status-success-bg'/>
                    <span className="text-xs">Self</span></div>
                  <div className="flex items-center gap-1.5">
                    <div className='w-3 h-1.5 rounded-full bg-accent'/>
                    <span className="text-xs">Peers</span></div>
                  <div className="flex items-center gap-1.5">
                    <div className='w-3 h-1.5 rounded-full bg-accent'/>
                    <span className="text-xs">Manager</span></div>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Recognition Feed */}
            <Card className='md:col-span-1 bg-[var(--bg-card)] border-t-4 border-status-warning-border'>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-2">
                  <Zap className='h-4 w-4 text-status-warning-text'/> Recognition Pulse
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex gap-4 group cursor-pointer">
                    <div
                      className='flex-shrink-0 w-8 h-8 rounded-full bg-status-warning-bg flex items-center justify-center text-xs font-bold text-status-warning-text'>
                      {i === 1 ? 'JD' : i === 2 ? 'AL' : 'KS'}
                    </div>
                    <div>
                      <h5 className='text-xs font-bold group-hover:text-accent transition-colors'>Amazing Sprint
                        Finish!</h5>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <div className='flex items-center gap-0.5 text-[8px] text-status-warning-text font-bold'>
                ...
                <Button variant="ghost" className='w-full text-xs text-accent p-0 h-auto'>View All Kudos</Button>
              </CardContent>
            </Card>

            {/* Coaching Corner */}
            <Card
              className='md:col-span-2 bg-gradient-to-br from-accent-50 to-white'>
              <CardHeader>
                <div className="row-between">
                  <CardTitle className='text-accent flex items-center gap-2'>
                    <Users className="h-5 w-5"/> Development Trajectory
                  </CardTitle>
                  <Button variant="primary" size="sm" className='bg-accent hover:bg-accent'>Connect with
                    Coach</Button>
                </div>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  <div
                    className='p-4 bg-[var(--bg-input)] rounded-lg shadow-[var(--shadow-card)] border border-[var(--accent-primary)]'>
                    <h4 className='text-xs font-bold text-accent uppercase tracking-wider mb-2'>Growth
                      Opportunity</h4>
                  ...
                  <div
                    className='p-4 bg-[var(--bg-input)] rounded-lg shadow-[var(--shadow-card)] border border-[var(--accent-primary)]'>
                    <h4 className='text-xs font-bold text-status-success-text uppercase tracking-wider mb-2'>Peak
                      Performance</h4>
                    ...
                    <div className="mt-4 row-between">
                      ...
                      <Award className='h-4 w-4 text-status-warning-text'/>
                    </div>
                  </div>
                </div>
              </CardContent>
