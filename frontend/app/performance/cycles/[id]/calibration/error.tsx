...
import { RefreshCw, Home, Sliders } from 'lucide-react';
...
export default function CalibrationError({ error, reset }: ErrorProps) {
  useEffect(() => {
    handleError(error, { source: 'calibration-error-boundary', digest: error.digest });
  }, [error]);
        ...
        <Card className="w-full max-w-md bg-[var(--bg-card)]">
          <CardHeader className="text-center">
            <div className="mx-auto mb-4 h-12 w-12 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center">
              <Sliders className="h-6 w-6 text-red-600 dark:text-red-400" />
            </div>
