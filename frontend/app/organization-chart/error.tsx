...
import { RefreshCw, Home, Network } from 'lucide-react';
...
export default function OrganizationChartError({ error, reset }: ErrorProps) {
  useEffect(() => {
    handleError(error, { source: 'organization-chart-error-boundary', digest: error.digest });
  }, [error]);
        ...
        <Card className="w-full max-w-md bg-[var(--bg-card)]">
          <CardHeader className="text-center">
            <div className="mx-auto mb-4 h-12 w-12 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center">
              <Network className="h-6 w-6 text-red-600 dark:text-red-400" />
            </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => (window.location.href = '/organization-chart')}
                className="w-full"
              >
