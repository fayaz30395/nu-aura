      ...
      updateRunMutation.mutate(
        {id: selectedPayrollRun.id, data: data as PayrollRunRequest},
        {
          ...
          onError: (err: unknown) => {
            ...
            })?.response?.data?.message || 'Failed to save payroll run');
          },
        }
      );
    } else {
  ...
  const handleDeletePayrollRun = () => {
    ...
    deleteRunMutation.mutate(selectedPayrollRun.id, {
      ...
      onError: (err: unknown) => {
        setError((err as {
          response?: { data?: { message?: string } }
        })?.response?.data?.message || 'Failed to delete payroll run');
      },
    });
  };
  ...
  const handleProcessPayrollRun = (run: PayrollRun) => {
    processRunMutation.mutate(run.id, {
      onError: (err: unknown) => {
        setError((err as {
          response?: { data?: { message?: string } }
        })?.response?.data?.message || 'Failed to process payroll run');
      },
    });
  };
  ...
  const handleApprovePayrollRun = (run: PayrollRun) => {
    approveRunMutation.mutate(run.id, {
      onError: (err: unknown) => {
        setError((err as {
          response?: { data?: { message?: string } }
        })?.response?.data?.message || 'Failed to approve payroll run');
      },
    });
  };
      ...
      <PermissionGate permission={Permissions.PAYROLL_VIEW}
        ...
        >
          <div className="max-w-7xl mx-auto">
            {/* Header */}
            <div className="mb-8">
              <h1 className="text-xl font-bold skeuo-emboss">Payroll Runs</h1>
              <p className="text-[var(--text-secondary)] mt-2 skeuo-deboss">Create and manage payroll processing
                runs</p>
            </div>
