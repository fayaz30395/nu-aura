...
export default function NuMailError({ error, reset }: ErrorProps) {
  useEffect(() => {
    handleError(error, { source: 'nu-mail-error-boundary', digest: error.digest });
  }, [error]);
        ...
        <Card className="w-full max-w-md bg-[var(--bg-card)]">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => (window.location.href = '/nu-mail')}
                className="w-full"
              >
