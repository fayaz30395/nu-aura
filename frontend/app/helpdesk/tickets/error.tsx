...
import { RefreshCw, Home, Ticket } from 'lucide-react';
...
export default function TicketsError({ error, reset }: ErrorProps) {
  useEffect(() => {
    handleError(error, { source: 'helpdesk-tickets-error-boundary', digest: error.digest });
  }, [error]);
        ...
        <Card className="w-full max-w-md bg-[var(--bg-card)]">
          <CardHeader className="text-center">
            <div className="mx-auto mb-4 h-12 w-12 rounded-full bg-danger-100 dark:bg-danger-900/30 flex items-center justify-center">
              <Ticket className="h-6 w-6 text-danger-600 dark:text-danger-400" />
            </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => (window.location.href = '/helpdesk/tickets')}
                className="w-full"
              >
