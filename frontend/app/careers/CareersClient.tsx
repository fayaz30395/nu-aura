...
export default function CareersClient({initialJobs}: CareersClientProps) {
  ...
  const hasActiveFilter = !!(searchQuery || selectedDepartment || selectedLocation || selectedType);

  // Use React Query for client-side filtering; fall back to server-fetched initialJobs when no filters active
  const {data: clientJobs, isLoading} = usePublicJobs(filters);
  const jobs = hasActiveFilter ? (clientJobs ?? []) : (clientJobs ?? initialJobs);

  const departments = Array.from(new Set(initialJobs.map((j) => j.department)));
  const locations = Array.from(new Set(initialJobs.map((j) => j.location)));
    ...
    <PageTransition className="page-shell-centered">
      ...
      <div className="bg-gradient-to-br from-accent-700 via-accent-600 to-accent-700 py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <h1 className="text-5xl sm:text-6xl font-bold text-white mb-6" suppressHydrationWarning>
