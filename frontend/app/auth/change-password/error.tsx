...
export default function ChangePasswordError({error, reset}: ErrorProps) {
  useEffect(() => {
    handleError(error, {source: 'change-password-error-boundary', digest: error.digest});
  }, [error]);
        ...
        <Card className="page-shell-card float-subtle">
          <CardHeader className="text-center">
            <div className="mx-auto mb-4 h-12 w-12 rounded-full bg-danger-100 dark:bg-danger-900/30 flex items-center justify-center">
              <Lock className="h-6 w-6 text-danger-600 dark:text-danger-400"/>
            </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button variant="outline" onClick={() => (window.location.href = '/auth/login')} className="w-full">
                <Lock className="mr-2 h-4 w-4"/>Back to Login
              </Button>
