...
import {Home, RefreshCw, UserCog} from 'lucide-react';
...
export default function AdminProfileError({error, reset}: ErrorProps) {
  useEffect(() => {
    handleError(error, {source: 'admin-profile-error-boundary', digest: error.digest});
  }, [error]);
        ...
        <Card className="page-shell-card float-subtle">
          <CardHeader className="text-center">
            <div className="mx-auto mb-4 h-12 w-12 rounded-full bg-danger-100 dark:bg-danger-900/30 flex items-center justify-center">
              <UserCog className="h-6 w-6 text-danger-600 dark:text-danger-400"/>
            </div>
