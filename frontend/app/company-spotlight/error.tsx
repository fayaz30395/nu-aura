...
export default function CompanySpotlightError({ error, reset }: ErrorProps) {
  useEffect(() => {
    handleError(error, { source: 'company-spotlight-error-boundary', digest: error.digest });
  }, [error]);
        ...
        <Card className="w-full max-w-md bg-white dark:bg-surface-900">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => (window.location.href = '/company-spotlight')}
                className="w-full"
              >
                <Newspaper className="mr-2 h-4 w-4" />
                Back to Company Spotlight
              </Button>
              <Button
                variant="outline"
