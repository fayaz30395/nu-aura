...
import { AlertTriangle, RefreshCw, Home, UserMinus } from 'lucide-react';
...
export default function OffboardingError({ error, reset }: ErrorProps) {
  useEffect(() => {
    handleError(error, { source: 'offboarding-error-boundary', digest: error.digest });
  }, [error]);
