...
test.describe('Scheduled Reports Page', () => {
  ...
  test('should toggle filter between All, Active, and Inactive', async ({page}) => {
    ...
    await expect(activeButton).toHaveClass(/bg-accent-700/);

    // Click Inactive filter
    await inactiveButton.click();
    await expect(inactiveButton).toHaveClass(/bg-accent-700/);

    // Click All filter
    await allButton.click();
    await expect(allButton).toHaveClass(/bg-accent-700/);
  });
