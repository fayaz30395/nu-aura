...
class PaymentService {
  ...
  getStatusColor(status: PaymentStatus): string {
    const colors: Record<PaymentStatus, string> = {
      ...
      REVERSED: 'bg-[var(--bg-surface)] text-gray-700 dark:bg-surface-950/50 dark:text-gray-300',
    };
    return colors[status] || 'bg-[var(--bg-surface)] text-gray-700 dark:bg-surface-950/50 dark:text-gray-300';
  }
