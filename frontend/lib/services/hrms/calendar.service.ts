...
class CalendarService {
  ...
  getWeekRange(date: Date = new Date()): { start: string; end: string } {
    ...
    // Calculate week start (Sunday) without mutating curr
    const weekStart = new Date(curr.getFullYear(), curr.getMonth(), curr.getDate() - dayOfWeek);
    weekStart.setHours(0, 0, 0, 0);

    // Calculate week end (Saturday) from weekStart to preserve correct month
