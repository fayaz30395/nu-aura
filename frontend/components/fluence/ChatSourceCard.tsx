...
const typeConfig: Record<string, { icon: React.ElementType; label: string; color: string }> = {
  ...
  template: { icon: FileStack, label: 'Template', color: 'text-primary-500 bg-primary-50 dark:bg-primary-950/30' },
};
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-medium text-[var(--text-primary)] truncate group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
          {source.title}
        </p>
        {source.excerpt && (
