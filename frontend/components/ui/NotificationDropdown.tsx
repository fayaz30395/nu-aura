    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-main)]">
        <div className="flex items-center gap-2">
          ...
          {unreadCount > 0 && (
            <span className="inline-flex items-center justify-center h-5 min-w-[20px] px-1.5 text-xs font-semibold rounded-full bg-sky-100 text-sky-700 dark:bg-sky-900 dark:text-sky-300">
              {unreadCount}
            </span>
          )}
        </div>
        {unreadCount > 0 && (
          <button
            ...
            className={cn(
              ...
              'text-sky-700 dark:text-sky-400 hover:bg-sky-50 dark:hover:bg-sky-950/30',
      ...
      <div className="max-h-[400px] overflow-y-auto">
        {isLoading ? (
          <div className="flex items-center justify-center py-8">
            <div className="h-5 w-5 animate-spin rounded-full border-2 border-sky-700 border-t-transparent" />
          </div>
        ) : displayed.length === 0 ? (
              ...
              <li key={notification.id}>
                <button
                  ...
                  className={cn(
                    ...
                    'focus:outline-none focus:ring-2 focus:ring-inset focus:ring-sky-700',
                    'border-b border-[var(--border-main)] last:border-b-0',
                    !notification.isRead && 'bg-sky-50/50 dark:bg-sky-950/10'
                  )}
                >
                  {/* Unread indicator dot */}
                  <div className="mt-1.5 shrink-0">
                    {!notification.isRead ? (
                      <span className="block h-2 w-2 rounded-full bg-sky-700" />
                    ) : (
