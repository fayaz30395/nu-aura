...
import {ChevronDown, Save, Send, Star} from 'lucide-react';
import {
  useCreateScorecardMutation,
  useScorecardTemplates,
  useSubmitScorecardMutation,
} from '@/lib/hooks/queries/useScorecard';
import type {ScorecardCriterion, ScorecardRecommendation, ScorecardTemplate,} from '@/lib/types/hire/scorecard';
    ...
    <div className="rounded-lg border border-[var(--border-main)] bg-[var(--bg-card)] p-4">
      ...
      <div className="flex items-center justify-end gap-2 border-t border-[var(--border-subtle)] pt-4">
        ...
        <button
          ...
          disabled={isSaving}
          className="skeuo-button flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg bg-[var(--btn-primary-bg)] text-white cursor-pointer hover:bg-[var(--btn-primary-bg-hover)] active:translate-y-px disabled:opacity-50 disabled:cursor-not-allowed focus-visible:ring-2 focus-visible:ring-[var(--ring-primary)]"
        >
          <Send size={14}/>
          Submit
        </button>
