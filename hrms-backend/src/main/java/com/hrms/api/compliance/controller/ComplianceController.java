...
public class ComplianceController {
    ...
    public ResponseEntity<CompliancePolicy> createPolicy(@Valid @RequestBody CompliancePolicy policy) {
        return ResponseEntity.ok(complianceService.createPolicy(policy));
    }

    @PutMapping("/policies/{id}")
    @RequiresPermission(Permission.COMPLIANCE_MANAGE)
    @Operation(summary = "Update an existing policy")
    public ResponseEntity<CompliancePolicy> updatePolicy(
            @PathVariable UUID id,
            @Valid @RequestBody CompliancePolicy policy) {
        return ResponseEntity.ok(complianceService.updatePolicy(id, policy));
    }

    @PostMapping("/policies/{id}/publish")
    @RequiresPermission(Permission.COMPLIANCE_MANAGE)
    @Operation(summary = "Publish a draft policy")
    public ResponseEntity<CompliancePolicy> publishPolicy(@PathVariable UUID id) {
        return ResponseEntity.ok(complianceService.publishPolicy(id));
    }

    @PostMapping("/policies/{id}/archive")
    @RequiresPermission(Permission.COMPLIANCE_MANAGE)
    @Operation(summary = "Archive a policy")
    public ResponseEntity<CompliancePolicy> archivePolicy(@PathVariable UUID id) {
        return ResponseEntity.ok(complianceService.archivePolicy(id));
    }

    @PostMapping("/policies/{id}/new-version")
    @RequiresPermission(Permission.COMPLIANCE_MANAGE)
    @Operation(summary = "Create a new version of an existing policy")
    public ResponseEntity<CompliancePolicy> createNewVersion(@PathVariable UUID id) {
        return ResponseEntity.ok(complianceService.createNewVersion(id));
    }

    @GetMapping("/policies")
    @RequiresPermission(Permission.COMPLIANCE_VIEW)
    @Operation(summary = "Get all policies")
    public ResponseEntity<Page<CompliancePolicy>> getAllPolicies(Pageable pageable) {
        return ResponseEntity.ok(complianceService.getAllPolicies(pageable));
    }

    @GetMapping("/policies/{id}")
    @RequiresPermission(Permission.COMPLIANCE_VIEW)
    @Operation(summary = "Get a specific policy")
    public ResponseEntity<CompliancePolicy> getPolicy(@PathVariable UUID id) {
    ...
    public ResponseEntity<List<CompliancePolicy>> getActivePolicies() {
    ...
    public ResponseEntity<List<CompliancePolicy>> getPoliciesByCategory(
            @PathVariable CompliancePolicy.PolicyCategory category) {
        return ResponseEntity.ok(complianceService.getPoliciesByCategory(category));
    }
    ...
    public ResponseEntity<List<CompliancePolicy>> getPendingAcknowledgments(@PathVariable UUID employeeId) {
        return ResponseEntity.ok(complianceService.getPendingAcknowledgments(employeeId));
    }
    ...
    public ResponseEntity<ComplianceChecklist> createChecklist(@Valid @RequestBody ComplianceChecklist checklist) {
        return ResponseEntity.ok(complianceService.createChecklist(checklist));
    }

    @PutMapping("/checklists/{id}")
    @RequiresPermission(Permission.COMPLIANCE_MANAGE)
    @Operation(summary = "Update a checklist")
    public ResponseEntity<ComplianceChecklist> updateChecklist(
            @PathVariable UUID id,
            @Valid @RequestBody ComplianceChecklist checklist) {
        return ResponseEntity.ok(complianceService.updateChecklist(id, checklist));
    }

    @PostMapping("/checklists/{id}/complete")
    @RequiresPermission(Permission.COMPLIANCE_MANAGE)
    @Operation(summary = "Complete a checklist")
    public ResponseEntity<ComplianceChecklist> completeChecklist(@PathVariable UUID id) {
        return ResponseEntity.ok(complianceService.completeChecklist(id));
    }

    @GetMapping("/checklists")
    @RequiresPermission(Permission.COMPLIANCE_VIEW)
    @Operation(summary = "Get all checklists")
    public ResponseEntity<Page<ComplianceChecklist>> getAllChecklists(Pageable pageable) {
        return ResponseEntity.ok(complianceService.getAllChecklists(pageable));
    }

    @GetMapping("/checklists/active")
    @RequiresPermission(Permission.COMPLIANCE_VIEW)
    @Operation(summary = "Get active checklists")
    public ResponseEntity<List<ComplianceChecklist>> getActiveChecklists() {
        return ResponseEntity.ok(complianceService.getActiveChecklists());
    }

    @GetMapping("/checklists/my")
    @RequiresPermission(Permission.COMPLIANCE_VIEW)
    @Operation(summary = "Get my assigned checklists")
    public ResponseEntity<List<ComplianceChecklist>> getMyChecklists() {
        return ResponseEntity.ok(complianceService.getMyChecklists());
    }

    @GetMapping("/checklists/overdue")
    @RequiresPermission(Permission.COMPLIANCE_VIEW)
    @Operation(summary = "Get overdue checklists")
    public ResponseEntity<List<ComplianceChecklist>> getOverdueChecklists() {
        return ResponseEntity.ok(complianceService.getOverdueChecklists());
    }
    ...
    public ResponseEntity<ComplianceAlert> createAlert(@Valid @RequestBody ComplianceAlert alert) {
        return ResponseEntity.ok(complianceService.createAlert(alert));
    }
    ...
    public ResponseEntity<ComplianceAlert> escalateAlert(@PathVariable UUID id) {
        return ResponseEntity.ok(complianceService.escalateAlert(id));
    }

    @GetMapping("/alerts")
    @RequiresPermission(Permission.COMPLIANCE_VIEW)
    @Operation(summary = "Get all alerts")
    public ResponseEntity<Page<ComplianceAlert>> getAllAlerts(Pageable pageable) {
        return ResponseEntity.ok(complianceService.getAllAlerts(pageable));
    }

    @GetMapping("/alerts/active")
    @RequiresPermission(Permission.COMPLIANCE_VIEW)
    @Operation(summary = "Get active alerts")
    public ResponseEntity<List<ComplianceAlert>> getActiveAlerts() {
        return ResponseEntity.ok(complianceService.getActiveAlerts());
    }
    ...
    public ResponseEntity<List<ComplianceAlert>> getMyAlerts() {
        return ResponseEntity.ok(complianceService.getMyAlerts());
    }
    ...
    public ResponseEntity<List<ComplianceAlert>> getCriticalAlerts() {
        return ResponseEntity.ok(complianceService.getCriticalAlerts());
    }
