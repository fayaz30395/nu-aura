...
public interface Feedback360CycleRepository extends JpaRepository<Feedback360Cycle, UUID> {
    ...
    @Query("SELECT c FROM Feedback360Cycle c WHERE c.tenantId = :tenantId AND c.status IN (com.hrms.domain.performance.Feedback360Cycle.CycleStatus.IN_PROGRESS, com.hrms.domain.performance.Feedback360Cycle.CycleStatus.NOMINATION_OPEN)")
