...
/**
 * Whitelist DTO for {@code POST /api/v1/payroll/payslips}.
 *
 * <p>Replaces direct {@code @RequestBody Payslip} binding to close mass-assignment
 * vector R-1.10. Allows only the fields the service's create logic legitimately
 * needs. System fields (id, tenantId, totals — recomputed via
 * {@code Payslip.calculateTotals()} — and pdfFileId) are NEVER accepted.</p>
 */
@Data
@JsonInclude(JsonInclude.Include.NON_NULL)
@Schema(description = "Request body for creating a payslip. " +
        "Totals are computed server-side; tenantId / pdfFileId / status fields are rejected.")
public class CreatePayslipRequest {

    @NotNull(message = "Payroll run id is required")
    @Schema(description = "Parent payroll run id", required = true)
    private UUID payrollRunId;

    @NotNull(message = "Employee id is required")
    @Schema(description = "Employee the payslip belongs to", required = true)
