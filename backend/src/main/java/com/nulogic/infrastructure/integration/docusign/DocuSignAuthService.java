...
public class DocuSignAuthService {
    ...
    public String getAccessToken(ConnectorConfig config) {
        ...
        // Fast path: lock-free read of a still-valid cached token.
        ...
        // Slow path: serialize the refresh per tenant so concurrent callers do not
        // trigger redundant OAuth flows. compute() holds the per-key lock, so the
        // re-check inside the mapping function is a double-checked lock — a token
        // generated by a racing thread is reused instead of regenerated.
        // DocuSign only allows one active JWT token per integration, so a second
        // redundant refresh would 401 in-flight requests using the first token.
        TokenCache refreshed = tokenCache.compute(tenantId, (key, existing) -> {
            if (existing != null && existing.isValid()) {
                return existing;
            }
            log.debug("Generating new DocuSign access token for tenant: {}", key);
            ...
            return new TokenCache(newToken, expiresAtMs);
        });
