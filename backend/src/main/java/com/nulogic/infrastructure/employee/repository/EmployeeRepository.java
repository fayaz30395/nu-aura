package com.nulogic.infrastructure.employee.repository;
...
public interface EmployeeRepository extends JpaRepository<Employee, UUID>, JpaSpecificationExecutor<Employee> {
    ...
    /**
     * Find all employees with eager loading of user, department, location, and
     * manager.
    ...
    Page<Employee> searchEmployees(@Param("tenantId") UUID tenantId, @Param("search") String search,
                                   Pageable pageable);
    ...
    List<Employee> findDirectReportsByManagerId(@Param("tenantId") UUID tenantId,
    ...
    List<UUID> findEmployeeIdsByManagerIds(@Param("tenantId") UUID tenantId,
    ...
    Long countByTenantIdAndIdInAndJoiningDateBetween(@Param("tenantId") UUID tenantId,
                                                     @Param("employeeIds") List<UUID> employeeIds, @Param("startDate") LocalDate startDate,
    ...
    List<Employee> findByTenantIdAndDepartmentIdIn(@Param("tenantId") UUID tenantId,
    ...
    List<Employee> findByTenantIdAndOfficeLocationIdIn(@Param("tenantId") UUID tenantId,
                                                       @Param("locationIds") Set<UUID> locationIds);
    ...
    /**
     * Batch query: returns termination counts grouped by (year, month) for a date
     * range.
     * Each element is Object[]{year (Integer), month (Integer), count (Long)}.
    ...
    List<Employee> findByTenantIdAndJoiningDateBetweenAndStatus(@Param("tenantId") UUID tenantId,
    ...
    List<Employee> findUpcomingBirthdays(@Param("tenantId") UUID tenantId, @Param("startDate") LocalDate startDate,
                                         @Param("endDate") LocalDate endDate);
    ...
    @Query(value = "SELECT e.id, e.first_name, e.middle_name, e.last_name, e.joining_date, " +
            ...
            ") " +
            "ORDER BY EXTRACT(MONTH FROM e.joining_date), EXTRACT(DAY FROM e.joining_date)", nativeQuery = true)
    List<Object[]> findUpcomingAnniversariesWithDepartment(@Param("tenantId") UUID tenantId,
                                                           @Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);

    /**
     * Find employees where the given manager is assigned as dotted-line manager 1
     * or 2.
    ...
    @Query("SELECT e FROM Employee e WHERE e.tenantId = :tenantId " +
            ...
            "AND e.status = com.nulogic.domain.employee.Employee.EmployeeStatus.ACTIVE " +
     ...
     * Employee.user is a @OneToOne(LAZY) association, so direct access will cause
     ...
     * Commonly used in authentication flows to load employee details for a
     * logged-in user.
     */
    @Query("SELECT DISTINCT e FROM Employee e " +
     ...
     * Returns ACTIVE employees only to prevent assigning a terminated person as
     * manager.
     */
    @Query("SELECT e FROM Employee e " +
     ...
     * Returns Object[]{UUID tenantId, Long count} for each tenant in the provided
     * set.
     * Replaces N per-tenant countByTenantId() calls in paginated list views (N+1
     * fix).
     */
    ...
    /**
     * Count employees who joined on or before the given date, falling back to
     * createdAt when
     * joiningDate is null. Used by getGrowthMetrics() to avoid loading all
     * employees into memory.
     */
    ...
    /**
     * Tenant-aware variant of
     * {@link #countJoinedOnOrBefore(LocalDate, LocalDateTime)}.
     ...
     * Used by ImplicitRoleEngine to determine if employee should get
     * IS_DEPARTMENT_HEAD role.
     */
     ...
     * A skip-level report is an employee whose manager has this employee as their
     * manager.
     * Used by ImplicitRoleEngine to determine if employee should get
     * IS_SKIP_LEVEL_MANAGER role.
     */
    @Query(value = """
            SELECT EXISTS(
