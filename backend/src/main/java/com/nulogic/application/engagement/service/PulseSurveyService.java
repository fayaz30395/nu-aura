package com.nulogic.application.engagement.service;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.nulogic.api.engagement.dto.PulseSurveyRequest;
...
import com.nulogic.domain.engagement.PulseSurveyQuestion;
import com.nulogic.domain.engagement.PulseSurveyQuestion.QuestionCategory;
import com.nulogic.domain.engagement.PulseSurveyQuestion.QuestionType;
import com.nulogic.infrastructure.engagement.repository.PulseSurveyAnswerRepository;
import com.nulogic.infrastructure.engagement.repository.PulseSurveyQuestionRepository;
import com.nulogic.infrastructure.engagement.repository.PulseSurveyRepository;
import com.nulogic.infrastructure.engagement.repository.PulseSurveyResponseRepository;
...
public class PulseSurveyService {
    ...
    public PulseSurvey closeSurvey(UUID surveyId, UUID closedBy) {
        ...
        Long submittedCount = responseRepository.countBySurveyIdAndStatus(surveyId,
                com.nulogic.domain.engagement.PulseSurveyResponse.ResponseStatus.SUBMITTED);
    ...
    @Transactional
    public com.nulogic.domain.engagement.PulseSurveyResponse startSurveyResponse(UUID surveyId, UUID employeeId) {
        UUID tenantId = TenantContext.getCurrentTenant();
        ...
        Optional<com.nulogic.domain.engagement.PulseSurveyResponse> existing =
                responseRepository.findBySurveyIdAndEmployeeId(surveyId, employeeId);
        if (existing.isPresent()) {
            return existing.get();
        }

        com.nulogic.domain.engagement.PulseSurveyResponse response =
                com.nulogic.domain.engagement.PulseSurveyResponse.builder()
                        .surveyId(surveyId)
    ...
    @Transactional
    public com.nulogic.domain.engagement.PulseSurveyResponse submitSurveyResponse(
            SurveySubmissionRequest request, UUID employeeId, String ipAddress) {
        UUID tenantId = TenantContext.getCurrentTenant();

        com.nulogic.domain.engagement.PulseSurveyResponse response = responseRepository
    ...
    public Map<String, Object> getSurveyAnalytics(UUID surveyId) {
        ...
        Long inProgress = responseRepository.countBySurveyIdAndStatus(surveyId,
                com.nulogic.domain.engagement.PulseSurveyResponse.ResponseStatus.IN_PROGRESS);
        Long invited = responseRepository.countBySurveyIdAndStatus(surveyId,
                com.nulogic.domain.engagement.PulseSurveyResponse.ResponseStatus.INVITED);
