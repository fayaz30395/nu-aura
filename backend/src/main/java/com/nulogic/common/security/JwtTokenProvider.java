...
public class JwtTokenProvider {
    ...
    public String generateTokenWithAppPermissions(User user, UUID tenantId, String appCode,
                                                  Map<String, com.nulogic.domain.user.RoleScope> permissions, Set<String> roles,
    ...
    public Map<String, com.nulogic.domain.user.RoleScope> getPermissionScopesFromToken(String token) {
        Claims claims = getClaims(token);
        Map<String, String> scopesMap = claims.get("permissionScopes", Map.class);
        if (scopesMap == null)
            return Collections.emptyMap();

        Map<String, com.nulogic.domain.user.RoleScope> result = new HashMap<>();
        scopesMap.forEach((perm, scope) -> result.put(perm, com.nulogic.domain.user.RoleScope.valueOf(scope)));
        return result;
    }
