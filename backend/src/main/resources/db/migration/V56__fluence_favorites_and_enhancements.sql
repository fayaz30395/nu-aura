...
(
  ...
  CONSTRAINT uq_fluence_favorites UNIQUE
(
  tenant_id,
  user_id,
  content_id,
...
ALTER TABLE wiki_page_comments
  ADD COLUMN IF NOT EXISTS parent_comment_id UUID REFERENCES wiki_page_comments(id);
...
ALTER TABLE wiki_spaces
  ADD COLUMN IF NOT EXISTS approval_enabled BOOLEAN DEFAULT FALSE;
ALTER TABLE wiki_spaces
...
(
  ...
  tenant_id UUID NOT NULL,
  wiki_page_id UUID NOT NULL REFERENCES wiki_pages
(
  id
),
...
(
  ...
  is_deleted BOOLEAN NOT NULL DEFAULT FALSE,
  deleted_at TIMESTAMPTZ,
  CONSTRAINT uq_wiki_page_likes UNIQUE
(
  tenant_id,
  wiki_page_id,
  liked_by
...
CREATE
POLICY fluence_favorites_tenant_isolation ON fluence_favorites
    USING (tenant_id = current_setting('app.current_tenant_id')::UUID);

ALTER TABLE fluence_activities ENABLE ROW LEVEL SECURITY;
CREATE
POLICY fluence_activities_tenant_isolation ON fluence_activities
    USING (tenant_id = current_setting('app.current_tenant_id')::UUID);

ALTER TABLE wiki_page_likes ENABLE ROW LEVEL SECURITY;
CREATE
POLICY wiki_page_likes_tenant_isolation ON wiki_page_likes
    USING (tenant_id = current_setting('app.current_tenant_id')::UUID);
