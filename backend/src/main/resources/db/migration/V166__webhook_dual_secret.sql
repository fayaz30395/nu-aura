...
ALTER TABLE webhooks ADD COLUMN previous_secret VARCHAR(255);
ALTER TABLE webhooks ADD COLUMN previous_secret_expires_at TIMESTAMP;

COMMENT ON COLUMN webhooks.previous_secret IS 'Previous HMAC secret kept for a rotation window so in-flight retries can still verify. NULL once rotation window expires.';
