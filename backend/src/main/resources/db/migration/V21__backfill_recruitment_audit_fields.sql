...
ALTER TABLE candidates ADD COLUMN IF NOT EXISTS version BIGINT DEFAULT 0;
ALTER TABLE candidates ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN NOT NULL DEFAULT FALSE;

-- ── job_openings ──
ALTER TABLE job_openings ADD COLUMN IF NOT EXISTS created_by UUID;
ALTER TABLE job_openings ADD COLUMN IF NOT EXISTS updated_by UUID;
ALTER TABLE job_openings ADD COLUMN IF NOT EXISTS version BIGINT DEFAULT 0;
ALTER TABLE job_openings ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN NOT NULL DEFAULT FALSE;
...
ALTER TABLE interviews ADD COLUMN IF NOT EXISTS version BIGINT DEFAULT 0;
ALTER TABLE interviews ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN NOT NULL DEFAULT FALSE;

-- Backfill version for any existing rows (NULL → 0 for optimistic locking)
UPDATE candidates SET version = 0 WHERE version IS NULL;
UPDATE job_openings SET version = 0 WHERE version IS NULL;
UPDATE interviews SET version = 0 WHERE version IS NULL;
...
UPDATE job_openings SET updated_at = created_at WHERE updated_at IS NULL;
