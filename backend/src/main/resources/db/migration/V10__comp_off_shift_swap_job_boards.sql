...
CREATE TABLE IF NOT EXISTS comp_off_requests (
    ...
    comp_off_days   NUMERIC(4,2) NOT NULL,           -- e.g. 0.5 or 1.0
    status          VARCHAR(20) NOT NULL DEFAULT 'PENDING',  -- PENDING, APPROVED, REJECTED, CREDITED
    ...
    CONSTRAINT uq_comp_off_employee_date UNIQUE (tenant_id, employee_id, attendance_date)
);
...
CREATE TABLE IF NOT EXISTS job_board_postings (
    ...
    board_name      VARCHAR(50) NOT NULL,   -- NAUKRI, INDEED, LINKEDIN, SHINE, MONSTER
    external_job_id VARCHAR(200),           -- ID returned by the board API
    external_url    TEXT,                  -- Direct link to the posting on the board
    status          VARCHAR(20) NOT NULL DEFAULT 'PENDING', -- PENDING, ACTIVE, PAUSED, EXPIRED, FAILED
    posted_at       TIMESTAMP,
    ...
    created_at      TIMESTAMP NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMP NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_job_board_unique UNIQUE (tenant_id, job_opening_id, board_name)
);
...
CREATE TABLE IF NOT EXISTS attendance_regularization_config (
    ...
    default_regularize_to   VARCHAR(20) NOT NULL DEFAULT 'PRESENT',  -- PRESENT, HALF_DAY, WFH
...
CREATE TABLE IF NOT EXISTS resource_conflict_log (
    ...
    total_allocation_pct NUMERIC(5,2),      -- combined % at peak overlap
