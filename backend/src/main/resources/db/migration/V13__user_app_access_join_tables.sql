...
CREATE TABLE IF NOT EXISTS user_app_roles (
    ...
    CONSTRAINT fk_user_app_roles_access FOREIGN KEY (user_app_access_id)
        REFERENCES user_app_access(id) ON DELETE CASCADE,
    CONSTRAINT fk_user_app_roles_role FOREIGN KEY (role_id)
...
CREATE TABLE IF NOT EXISTS user_app_direct_permissions (
    ...
    CONSTRAINT fk_user_app_direct_perm_access FOREIGN KEY (user_app_access_id)
        REFERENCES user_app_access(id) ON DELETE CASCADE,
    CONSTRAINT fk_user_app_direct_perm_perm FOREIGN KEY (permission_id)
...
COMMENT ON TABLE user_app_roles IS 'Join table linking user app access to app roles';
COMMENT ON TABLE user_app_direct_permissions IS 'Join table for direct permission assignments (beyond role permissions)';
