...
(
  ...
  tenant_id UUID NOT NULL,
  template_id UUID NOT NULL REFERENCES scorecard_templates
(
  id
),
  name VARCHAR
...
(
  ...
  applicant_id UUID NOT NULL REFERENCES applicants
(
  id
),
...
(
  ...
  template_id UUID REFERENCES scorecard_templates
(
  id
),
...
(
  ...
  tenant_id UUID NOT NULL,
  scorecard_id UUID NOT NULL REFERENCES interview_scorecards
(
  id
),
  name VARCHAR
...
ALTER TABLE applicants
  ADD COLUMN IF NOT EXISTS veteran_status VARCHAR (30);
ALTER TABLE applicants
  ADD COLUMN IF NOT EXISTS disability_status VARCHAR (30);
...
FROM permissions p
WHERE
  p.code IN ('SCORECARD:VIEW', 'SCORECARD:CREATE', 'SCORECARD:UPDATE', 'SCORECARD:DELETE', 'SCORECARD:TEMPLATE_MANAGE')
