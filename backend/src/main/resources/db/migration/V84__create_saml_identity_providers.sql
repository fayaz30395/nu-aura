...
(
  ...
  -- Foreign keys
  CONSTRAINT fk_saml_idp_tenant FOREIGN KEY
(
  tenant_id
)
  REFERENCES tenants
(
  ...
  CONSTRAINT fk_saml_idp_default_role FOREIGN KEY
(
  default_role_id
)
  REFERENCES roles
...
CREATE
POLICY saml_identity_providers_tenant_isolation ON saml_identity_providers
...
COMMENT
ON TABLE saml_identity_providers IS 'SAML 2.0 Identity Provider configurations per tenant for enterprise SSO';
COMMENT
ON COLUMN saml_identity_providers.certificate IS 'IdP X.509 signing certificate (PEM), stored encrypted via EncryptionService';
COMMENT
ON COLUMN saml_identity_providers.attribute_mapping IS 'JSON mapping of SAML attributes to user fields (email, firstName, lastName, employeeId, department)';
