...
(
  ...
  CONSTRAINT uk_expense_cat_tenant_name UNIQUE
(
  tenant_id,
  name
),
  CONSTRAINT fk_expense_cat_parent FOREIGN KEY
(
  parent_category_id
)
...
(
  ...
  single_claim_limit NUMERIC
(
  12,
  2
),
  requires_pre_approval BOOLEAN NOT NULL DEFAULT FALSE,
  pre_approval_threshold NUMERIC
(
  12,
  2
),
  receipt_required_above NUMERIC
(
  12,
  2
),
...
(
  ...
  is_deleted BOOLEAN NOT NULL DEFAULT FALSE,
  deleted_at TIMESTAMP,
  CONSTRAINT uk_expense_policy_tenant_name UNIQUE
(
  tenant_id,
  name
)
...
(
  ...
  is_deleted BOOLEAN NOT NULL DEFAULT FALSE,
  deleted_at TIMESTAMP,
  CONSTRAINT fk_expense_item_claim FOREIGN KEY
(
  expense_claim_id
)
  REFERENCES expense_claims
(
  id
) ON DELETE CASCADE,
  CONSTRAINT fk_expense_item_category FOREIGN KEY
(
  category_id
)
  REFERENCES expense_categories
...
(
  ...
  is_deleted BOOLEAN NOT NULL DEFAULT FALSE,
  deleted_at TIMESTAMP,
  CONSTRAINT fk_expense_advance_settlement FOREIGN KEY
(
  settlement_claim_id
)
  REFERENCES expense_claims
...
END IF;
    ...
    IF
NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'expense_policies_tenant_isolation') THEN
        CREATE
...
END IF;
    ...
    IF
NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'expense_advances_tenant_isolation') THEN
        CREATE
...
FROM (SELECT id FROM tenants WHERE is_deleted = FALSE) t
       CROSS JOIN (VALUES ('Travel', 'Transportation, flights, fuel', TRUE, 'Plane', 1),
                          ...
                          ('Miscellaneous', 'Other business expenses', FALSE, 'MoreHorizontal',
                           7)) AS cat(name, description, requires_receipt, icon_name, sort_order)
