...
application_id UUID NOT NULL,
    ...
    CONSTRAINT uc_app_roles_code_tenantId UNIQUE (code, tenantId, application_id)
);
...
employee_id UUID NOT NULL,
    ...
    remarks TEXT,
    CONSTRAINT uc_employee_loans_tenantId_loan_number UNIQUE (tenantId, loan_number)
);
...
employee_id UUID NOT NULL,
    ...
    notes VARCHAR(1000),
    CONSTRAINT uc_expense_claims_tenantId_claim_number UNIQUE (tenantId, claim_number)
);
...
enrollment_id UUID NOT NULL,
    ...
    total_pages INTEGER,
    CONSTRAINT uc_lms_content_progress_enrollmentId_contentId UNIQUE (enrollmentId, contentId, tenantId)
);
...
course_id UUID NOT NULL,
    ...
    CONSTRAINT uc_lms_course_enrollments_courseId_employeeId UNIQUE (courseId, employeeId, tenantId)
);
...
channel VARCHAR(50) NOT NULL,
    ...
    CONSTRAINT uc_notification_channel_configs_tenantId_channel UNIQUE (tenantId, channel)
);
...
application_id UUID NOT NULL,
    ...
    max_users INTEGER,
    configuration TEXT,
    CONSTRAINT uc_tenant_applications_tenantId_application_id UNIQUE (tenantId, application_id)
);
...
user_id UUID NOT NULL,
    ...
    CONSTRAINT uc_user_notification_preferences_userId_category UNIQUE (userId, category)
);
