...
BEGIN
    IF
EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'approval_tasks') THEN
...
BEGIN
    IF
EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'approval_instances') THEN
...
SET tenant_id = u.tenant_id FROM users u
...
SET tenant_id = r.tenant_id FROM roles r
