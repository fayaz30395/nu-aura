...
CREATE TABLE IF NOT EXISTS statutory_filing_templates (
    ...
    CONSTRAINT uq_sft_tenant_type UNIQUE (tenant_id, filing_type)
);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'statutory_filing_templates' AND policyname = 'sft_tenant_isolation') THEN
        CREATE POLICY sft_tenant_isolation ON statutory_filing_templates
            USING (tenant_id = current_setting('app.current_tenant')::uuid);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'statutory_filing_runs' AND policyname = 'sfr_tenant_isolation') THEN
        CREATE POLICY sfr_tenant_isolation ON statutory_filing_runs
            USING (tenant_id = current_setting('app.current_tenant')::uuid);
    END IF;
END $$;
...
FROM (
    VALUES
        ('PF_ECR', 'PF ECR', 'Electronic Challan-cum-Return for EPFO portal upload', 'TEXT'),
        ...
        ('LWF_RETURN', 'LWF Return', 'Labour Welfare Fund semi-annual contribution return', 'CSV')
) AS ft(filing_type, name, description, format)
CROSS JOIN (
    SELECT DISTINCT id FROM statutory_filing_templates
    UNION
    SELECT id FROM (SELECT gen_random_uuid() AS id WHERE FALSE) AS empty
) AS t(id)
