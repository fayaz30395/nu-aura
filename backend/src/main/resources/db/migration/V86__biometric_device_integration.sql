...
ALTER TABLE biometric_devices
  ADD CONSTRAINT uk_biometric_device_serial_tenant UNIQUE (serial_number, tenant_id);
...
ALTER TABLE biometric_punch_logs
  ADD CONSTRAINT fk_punch_log_device
    FOREIGN KEY (device_id) REFERENCES biometric_devices (id);
ALTER TABLE biometric_punch_logs
  ADD CONSTRAINT fk_punch_log_attendance
...
ALTER TABLE biometric_api_keys
  ADD CONSTRAINT uk_biometric_api_key_hash UNIQUE (key_hash);

-- Foreign key to device (optional — key may be tenant-wide)
ALTER TABLE biometric_api_keys
  ADD CONSTRAINT fk_api_key_device
    FOREIGN KEY (device_id) REFERENCES biometric_devices (id);
