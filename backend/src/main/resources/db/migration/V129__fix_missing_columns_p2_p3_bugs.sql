...
BEGIN
    ...
    IF
EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'feedback_360_cycles') THEN
...
END IF;
    ...
    IF
EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'pulse_surveys') THEN
...
END IF;
    ...
    IF
EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'one_on_one_meetings') THEN
...
END IF;
    ...
    IF
EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'surveys') THEN
        EXECUTE 'ALTER TABLE surveys ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ';
END IF;

    RAISE
NOTICE 'V129: Fixed deleted_at columns and one_on_one_meetings time column types';
END $$;
