...
ALTER TABLE candidates ADD CONSTRAINT candidates_status_check
  CHECK (status IN ('NEW','SCREENING','INTERVIEW','SELECTED','OFFER_EXTENDED','OFFER_ACCEPTED','OFFER_DECLINED','REJECTED','WITHDRAWN'));

ALTER TABLE candidates ADD CONSTRAINT candidates_source_check
  CHECK (source IS NULL OR source IN ('JOB_PORTAL','REFERRAL','LINKEDIN','COMPANY_WEBSITE','WALK_IN','CAMPUS','CONSULTANT','OTHER'));

ALTER TABLE job_openings ADD CONSTRAINT job_openings_employment_type_check
  CHECK (employment_type IS NULL OR employment_type IN ('FULL_TIME','PART_TIME','CONTRACT','TEMPORARY','INTERNSHIP'));

ALTER TABLE job_openings ADD CONSTRAINT job_openings_status_check
  CHECK (status IN ('DRAFT','OPEN','ON_HOLD','CLOSED','CANCELLED'));

ALTER TABLE job_openings ADD CONSTRAINT job_openings_priority_check
...
ALTER TABLE interviews ADD CONSTRAINT interviews_result_check
...
UPDATE candidates SET current_stage = 'RECRUITERS_PHONE_CALL'        WHERE current_stage = 'APPLICATION_RECEIVED';
UPDATE candidates SET current_stage = 'PANEL_REVIEW'                 WHERE current_stage = 'SCREENING';
UPDATE candidates SET current_stage = 'TECHNICAL_INTERVIEW_SCHEDULED' WHERE current_stage = 'TECHNICAL_ROUND';
UPDATE candidates SET current_stage = 'HR_FINAL_INTERVIEW_COMPLETED' WHERE current_stage = 'HR_ROUND';
UPDATE candidates SET current_stage = 'MANAGEMENT_INTERVIEW_SCHEDULED' WHERE current_stage = 'MANAGER_ROUND';
UPDATE candidates SET current_stage = 'CLIENT_INTERVIEW_COMPLETED'   WHERE current_stage = 'FINAL_ROUND';
UPDATE candidates SET current_stage = 'OFFER_NDA_TO_BE_RELEASED'     WHERE current_stage = 'OFFER';
UPDATE candidates SET current_stage = 'OFFER_NDA_TO_BE_RELEASED'     WHERE current_stage = 'JOINED';
