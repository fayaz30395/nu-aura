...
INSERT INTO project_members (id, tenant_id, created_at, updated_at,
...
SELECT pe.id,
       ...
       COALESCE(pe.allocation_percentage, 0)::NUMERIC(5,2), COALESCE(pe.start_date, CURRENT_DATE),
       pe.end_date,
...
CREATE
OR REPLACE VIEW project_employees AS
SELECT id,
       tenant_id,
       created_at,
       updated_at,
       project_id,
       employee_id,
       role::VARCHAR(100)            AS role, allocation_percentage::INTEGER AS allocation_percentage, start_date,
       end_date,
       is_active
