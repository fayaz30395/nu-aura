...
CREATE
POLICY rls_implicit_role_rules ON implicit_role_rules
...
CREATE
POLICY rls_implicit_user_roles ON implicit_user_roles
    USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
...
CREATE
POLICY rls_approval_escalation_config ON approval_escalation_config
    USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
