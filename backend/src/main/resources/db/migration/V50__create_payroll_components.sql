...
COMMENT ON TABLE payroll_components IS 'Formula-based salary components evaluated in dependency order (DAG). Circular dependencies are detected at the application layer via topological sort.';
COMMENT ON COLUMN payroll_components.code IS 'Unique identifier within tenant, referenced in other components formulas (e.g., basic, hra, pf_employee)';
COMMENT ON COLUMN payroll_components.formula IS 'SpEL expression referencing other component codes (e.g., basic * 0.4). NULL means value is provided directly.';
COMMENT ON COLUMN payroll_components.evaluation_order IS 'Computed by topological sort. Lower values are evaluated first (no dependencies).';
