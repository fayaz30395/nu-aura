...
CREATE POLICY integration_connector_configs_allow_all ON integration_connector_configs
    AS PERMISSIVE FOR ALL
    USING (true)
    WITH CHECK (true);

CREATE POLICY integration_event_log_allow_all ON integration_event_log
    AS PERMISSIVE FOR ALL
    USING (true)
    WITH CHECK (true);

CREATE POLICY docusign_envelopes_allow_all ON docusign_envelopes
    AS PERMISSIVE FOR ALL
    USING (true)
    WITH CHECK (true);

CREATE POLICY docusign_template_mappings_allow_all ON docusign_template_mappings
    AS PERMISSIVE FOR ALL
    USING (true)
    WITH CHECK (true);
...
CREATE POLICY integration_connector_configs_tenant_rls ON integration_connector_configs
...
CREATE POLICY integration_event_log_tenant_rls ON integration_event_log
    AS RESTRICTIVE FOR ALL
    USING (
        tenant_id = current_setting('app.current_tenant_id', true)::uuid
        OR current_setting('app.current_tenant_id', true) IS NULL
...
CREATE POLICY docusign_envelopes_tenant_rls ON docusign_envelopes
    AS RESTRICTIVE FOR ALL
    USING (
        tenant_id = current_setting('app.current_tenant_id', true)::uuid
        OR current_setting('app.current_tenant_id', true) IS NULL
...
CREATE POLICY docusign_template_mappings_tenant_rls ON docusign_template_mappings
    AS RESTRICTIVE FOR ALL
    USING (
        tenant_id = current_setting('app.current_tenant_id', true)::uuid
        OR current_setting('app.current_tenant_id', true) IS NULL
...
VALUES (
    '660e8401-0001-0001-0001-000000000501',
    'integration.read',
    'View Integrations',
...
INSERT INTO permissions (id, code, name, description, resource, action, created_at, updated_at, version, is_deleted)
VALUES (
    '660e8401-0001-0001-0001-000000000502',
    'integration.manage',
    'Manage Integrations',
