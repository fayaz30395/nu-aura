...
WHERE role_id IN (
    SELECT id FROM app_roles WHERE name = 'EMPLOYEE' AND is_deleted = false
)
AND permission_id IN (
    SELECT id FROM permissions WHERE code = 'RECRUITMENT:VIEW' AND is_deleted = false
);
...
AND permission_id IN (
    SELECT id FROM permissions WHERE code = 'OFFBOARDING:VIEW' AND is_deleted = false
);
...
DELETE FROM role_permissions
WHERE role_id IN (
    SELECT id FROM app_roles WHERE name = 'TEAM_LEAD' AND is_deleted = false
)
AND permission_id IN (
