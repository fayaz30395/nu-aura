...
DECLARE
    ...
    i INT;
BEGIN
    SELECT id, tenant_id INTO v_post_id, v_tenant_id
    FROM social_posts
    ...
    RAISE NOTICE 'Post: % (tenant: %)', v_post_id, v_tenant_id;

    FOR i IN 1..8 LOOP
        v_email := lower(v_names[i][1]) || '.' || lower(v_names[i][2]) || '@nulogic.io';
        v_emp_code := 'EMP-' || LPAD((1000 + i)::TEXT, 4, '0');

        -- Check if user already exists
        SELECT id INTO v_existing_user_id
        FROM users WHERE email = v_email AND tenant_id = v_tenant_id;
        ...
        ELSE
            ...
            RAISE NOTICE 'User exists: %', v_email;
        END IF;

        -- Check if employee already exists
        SELECT id INTO v_existing_emp_id
        FROM employees WHERE employee_code = v_emp_code AND tenant_id = v_tenant_id;
        ...
        IF v_existing_emp_id IS NULL THEN
            ...
            INSERT INTO employees (
                ...
                v_emp_id, v_tenant_id, v_user_id, v_emp_code,
                v_names[i][1], v_names[i][2], v_designations[i],
                'ACTIVE', 'FULL_TIME', (NOW() - interval '180 days')::date,
                false, 0, NOW() - interval '30 days', NOW()
            );
            RAISE NOTICE 'Created employee: % %', v_names[i][1], v_names[i][2];
        ELSE
            v_emp_id := v_existing_emp_id;
            RAISE NOTICE 'Employee exists: % %', v_names[i][1], v_names[i][2];
        END IF;

        -- Add like if not already liked
        IF NOT EXISTS (SELECT 1 FROM post_reactions WHERE post_id = v_post_id AND employee_id = v_emp_id) THEN
            ...
            v_count := v_count + 1;
            RAISE NOTICE 'Added like from: % %', v_names[i][1], v_names[i][2];
        ELSE
    ...
    RAISE NOTICE 'Done! Added % new likes. Total: %',
        v_count,
